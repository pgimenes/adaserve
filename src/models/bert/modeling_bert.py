...
from nn.manual import ManualLinear, ManualLayerNorm
...
class BertEmbeddings(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.LayerNorm = ManualLayerNorm(
...
class BertSelfAttention(nn.Module):
    def __init__(self, config, position_embedding_type=None):
        ...
        self.query = ManualLinear(config.hidden_size, self.all_head_size)
        self.key = ManualLinear(config.hidden_size, self.all_head_size)
        self.value = ManualLinear(config.hidden_size, self.all_head_size)
...
class BertSelfOutput(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.dense = ManualLinear(config.hidden_size, config.hidden_size)
        self.LayerNorm = ManualLayerNorm(
            config.hidden_size, eps=config.layer_norm_eps, elementwise_affine=False
        )
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
...
class BertIntermediate(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.dense = ManualLinear(config.hidden_size, config.intermediate_size)
...
class BertOutput(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.dense = ManualLinear(config.intermediate_size, config.hidden_size)
        self.LayerNorm = ManualLayerNorm(
            config.hidden_size, eps=config.layer_norm_eps, elementwise_affine=False
        )
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
...
class BertLMPredictionHead(nn.Module):
    def __init__(self, config):
        ...
        self.decoder = ManualLinear(config.hidden_size, config.vocab_size, bias=False)
...
class BertOnlyNSPHead(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.seq_relationship = ManualLinear(config.hidden_size, 2)
...
class BertPreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module):
        ...
        if isinstance(module, ManualLinear):
        ...
        elif isinstance(module, ManualLayerNorm):
            if module.bias is not None:
                module.bias.data.zero_()
...
class BertForSequenceClassification(BertPreTrainedModel):
    def __init__(self, config):
        ...
        self.classifier = ManualLinear(config.hidden_size, config.num_labels)
...
class BertForMultipleChoice(BertPreTrainedModel):
    def __init__(self, config):
        ...
        self.dropout = nn.Dropout(classifier_dropout)
        self.classifier = ManualLinear(config.hidden_size, 1)
...
class BertForQuestionAnswering(BertPreTrainedModel):
    def __init__(self, config):
        ...
        self.bert = BertModel(config, add_pooling_layer=False)
        self.qa_outputs = ManualLinear(config.hidden_size, config.num_labels)
