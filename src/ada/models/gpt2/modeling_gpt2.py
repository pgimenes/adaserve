...
class GPT2Block(nn.Module):
    def __init__(self, config, layer_idx=None):
        ...
        self.ln_1 = ManualLayerNorm(hidden_size, eps=config.layer_norm_epsilon)
        self.attn = attention_class(config=config, layer_idx=layer_idx)
        self.ln_2 = ManualLayerNorm(hidden_size, eps=config.layer_norm_epsilon)

        if config.add_cross_attention:
            self.crossattention = attention_class(
                config=config, is_cross_attention=True, layer_idx=layer_idx
            )
            self.ln_cross_attn = ManualLayerNorm(
...
class GPT2Model(GPT2PreTrainedModel):
    def __init__(self, config):
        ...
        self.ln_f = ManualLayerNorm(self.embed_dim, eps=config.layer_norm_epsilon)
    ...
    def forward(
        ...
        if inputs_embeds is None:
            ...
            inputs_embeds = F.embedding(
                input=input_ids,
                weight=self.wte.weight,
                padding_idx=self.wte.padding_idx,
                max_norm=self.wte.max_norm,
                norm_type=self.wte.norm_type,
                scale_grad_by_freq=self.wte.scale_grad_by_freq,
                sparse=self.wte.sparse,
            )
        # position_embeds = self.wpe(position_ids)
        position_embeds = F.embedding(
            input=position_ids,
            weight=self.wpe.weight,
            padding_idx=self.wpe.padding_idx,
            max_norm=self.wpe.max_norm,
            norm_type=self.wpe.norm_type,
            scale_grad_by_freq=self.wpe.scale_grad_by_freq,
            sparse=self.wpe.sparse,
        )
...
class GPT2LMHeadModel(GPT2PreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.lm_head = ManualBatchLinear(config.n_embd, config.vocab_size, bias=False)
